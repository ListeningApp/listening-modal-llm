...
def train(run_folder: str, output_dir: str):
    ...
    with open(f'{run_folder}/logs.txt', 'a') as f:
        ...
        print(f'Beginning merge {merge_handle.object_id}.')  # type: ignore [union-attr]
    return merge_handle
...
def merge(run_folder: str, output_dir: str):
    ...
    VOLUME_CONFIG['/runs'].commit()  # type: ignore [union-attr]
...
def launch(name: str, config_raw: dict, data_raw: str, run_to_resume: str | None, preproc_only: bool):
    ...
    config = yaml.safe_load(config_raw)  # type: ignore [arg-type]
    ...
    try:
        ...
        VOLUME_CONFIG['/pretrained'].commit()  # type: ignore [union-attr]
    ...
    with (
        ...
        config_file.write(config_raw)  # type: ignore [arg-type]
    ...
    if preproc_only:
        ...
        with open(f'{run_folder}/logs.txt', 'w') as f:
            lbl = 'preproc'
            f.write(f'{lbl}: https://modal.com/logs/call/{preproc_handle.object_id}')  # type: ignore [union-attr]
        # wait for preprocessing to finish.
        preproc_handle.get()  # type: ignore [union-attr]
    ...
    with open(f'{run_folder}/logs.txt', 'w') as f:
        ...
        f.write(f'{lbl}: https://modal.com/logs/call/{launch_handle.object_id}')  # type: ignore [union-attr]
    VOLUME_CONFIG['/runs'].commit()  # type: ignore [union-attr]
...
def run_cmd(cmd: str, run_folder: str):
    ...
    VOLUME_CONFIG['/pretrained'].reload()  # type: ignore [union-attr]
    VOLUME_CONFIG['/runs'].reload()  # type: ignore [union-attr]
