...
One Statistical and scientific approach. This approach is concerned with determining how many patients are needed to get firm evidence of a treatment difference if it exists, and to estimate any difference precisely.

Two Economic and pragmatic approach. In reality, other constraints will impact on the ability to recruit the numbers required from a purely statistical approach. For example, how many patients are available, and how much time, effort and cost is involved?
...
In order to appreciate the impact of sample size for interpreting trial results consider the table below showing the results from two placebo-controlled trials assessing the impact of streptokinase for reducing mortality in patients who have experienced a myocardial infarction MI.

P is less than point zero zero zero one Risk ratio equals zero point seventy-seven, ninety-five percent confidence interval zero point seventy to zero point eighty-four. The estimated treatment effect is very similar in the two trials. Further, the proportions of patients who die in the two trials are also very similar. However, while the ISIS-two trial provides very strong evidence of a treatment effect with P is less than point zero zero zero one and a tight confidence interval, the first Australian trial is unconvincing with P equals zero point thirty-two and a wide confidence interval. Even though more than five hundred patients were recruited, the trial was too small in these circumstances.
...
## Four Type I and type II errors
...
p one equals the observed percentage in those on standard treatment observed percentage in those on 'new' treatment i.e. observed treatment effect (observed absolute percentage reduction on 'new' treatment)

percentage in those on standard treatment anticipated percentage in those on 'new' treatment i.e. true difference which has been decided it is important to detect

Further notations are as follows:

alpha equals significance level for detecting a difference (usually set at 0.05 or five percent)

one minus beta equals degree of certainty that a true difference would be detected i.e. the power (often aim for ninety percent power)
...
(ii) in the bottom right-hand cell of the table, i.e. if, in truth, a difference does exist (the alternative hypothesis), then evidence for a difference is observed in the trial.

These are marked by X in the table.


## Table two: Observed trial results compared to the 'truth' of (i) no difference

(ii) a true difference

X Three point four

However, there are two particular types of errors that can be made in interpreting trial results. These are referred to as type I and type II errors.
...
. Type II error. A type II error is when in truth there exists a difference of T one - seventy-two but the observed results fail to reach statistical significance, i.e. a false negative result.

Alternative ways of describing alpha and beta are as follows and represented in the table below:
...
In the UK PACE example the intended power was set at ninety percent. This means that if the true underlying percentages are twenty-four percent and eighteen percent, there is a ten percent probability that the observed results will give p greater than 0.05. This will occur if the observed difference is less than the true difference to the extent that it is more compatible with the null hypothesis of no difference. Some risk of a false negative result has to be accepted (otherwise an infinitely large study is needed).

Note that some risk of a false positive result also has to be accepted: it is quite possible that p less than 0.05 might be observed even if there is no true treatment difference.


## Three point five

Common choices for alpha (significance level) and one minus beta (power) are 0.05 and 0.90 respectively as already stated. Based on these choices, the equation to calculate the required sample size in each group for the comparison of two proportions is given below:
...
The formula given for the comparison for two proportions is based on a type one error of alpha equals zero point zero five and a type two error of beta equals zero point one. However, a more general formula for different values of alpha and beta is needed.

A more general formula is as follows:

where F of alpha and beta is a function of alpha and beta. The table below provides the value of F of alpha and beta for different values of alpha and beta.


## Three point six

Note that the value of F of alpha and beta can be calculated for any level of alpha and beta and this is given by the formula below. You will need to understand what Z one minus alpha over two and Z one minus beta represent: the z-distribution has a mean of zero with a standard deviation of one.

F of alpha and beta equals left parenthesis Z one minus alpha over two plus Z one minus beta right parenthesis squared. For example, for alpha equals zero point zero five and beta equals zero point one: F of alpha and beta equals left parenthesis one point nine six plus one point two eight right parenthesis squared equals ten point five. For example, for alpha equals zero point zero five and beta equals zero point two: F of alpha and beta equals left parenthesis one point nine six plus zero point eight four right parenthesis squared equals seven point eight five. Six. Impact of choice of scientific criteria on sample size

The key factors required in order to calculate the required sample size and needed for a trial comparing two percentages have been introduced. Assuming the primary outcome variable has been identified these are: one. The expected percentage or risk in the control arm. Two. The smallest clinically important difference that needs detecting. Three. The level of significance, and four. The power to detect the difference specified in point two if such a true difference exists.
...
F of alpha and beta equals ten point five. One. Impact of reducing the difference to detect.

Suppose one quarter reduction is considered unrealistically high and decide an absolute difference of three percent from twenty-four percent would clinically important to detect i.e. pi one minus pi two equals three percent. The total sample size required is eight thousand one hundred twenty-eight. Halving the absolute difference to detect for a given expected percentage outcome in the control arm resulted in more than quadrupling the sample size.
...
Suppose more stringent evidence for a treatment difference is required and the level of significance is set to be one percent i.e. require p is less than zero point zero one to claim a treatment difference. F of alpha and beta now becomes fourteen point nine and the total sample size needs to be increased to two thousand seven hundred thirty-two.

Four. Impact of increasing the power i.e. the risk of a type two error.

Suppose it is required for the study to give more chance of finding a treatment difference and ninety-five percent power is used. F of alpha and beta now becomes thirteen point zero and the total sample size needs to be increased to two thousand three hundred eighty-four.
...
To distinguish between means and percentages ut one and ut two will be used to refer to the anticipated means in the two treatment groups.
...
I one equals anticipated mean response on the standard treatment

Mu two equals anticipated mean response on the alternative treatment

Sigma equals standard deviation of response (assumed the same on both treatments)

Alpha equals risk of a type one error (significance level)

Beta equals risk of a type two error. Once these values have been decided upon, the required number per group can be calculated as follows:

N equals function of alpha, beta times two sigma squared divided by mu one minus mu two squared in each group. Note that the actual values of mu one and mu two are less important than the treatment difference to be detected. Suppose delta is the anticipated treatment difference then the above formula can be written as:

N equals function of alpha, beta times two sigma squared divided by delta squared in each group. Seven point two Example in kidney transplantation

The REPAIR trial was designed to assess the impact of remote ischaemic preconditioning using repeated inflations of a blood pressure cuff on kidney function in patients undergoing kidney transplantation. The outcome of interest is a glomerular filtration rate at one year.

The assumptions are as follows:

Delta equals four point seven three milliliters per minute per one point seventy-three square meters superiority with RIPC equals mu one minus mu two equivalent to a one thousand ninety-six improvement. Sigma equals thirteen point nine milliliters per minute per one point seventy-three square meters from a previous study

Alpha equals zero point zero five. Beta equals zero point two, i.e. power equals zero point eight or eighty percent. The number required in each group is two hundred twenty-two seven point eighty-five times two times thirteen point nine squared divided by four point seven three squared equals one hundred thirty-six, two hundred seventy-two in total.

To allow for losses to follow-up the trial requires approximately three hundred twenty patients.

It has been seen already when estimating the sample size for comparing percentages that as the absolute difference to detect decreases then the number required increases. In addition, as the significance level decreases or as the power increases this also leads to an increase in the numbers required. The same principles apply when comparing means as can be seen from the sample size formula.
...
The number required in each group becomes two hundred twenty-two seven point eighty-five times two times fourteen point nine squared divided by four point seven three squared equals one hundred fifty-six. Three hundred twelve in total.
...
The number required in each group would be two hundred twenty-two seven point eighty-five times two times twelve point nine squared divided by four point seven three squared equals one hundred seventeen, i.e. two hundred thirty-four in total.
...
A detailed discussion is beyond the scope of this session and requires an understanding of an analysis technique call 'analysis of covariance' (ANCOVA). Basically this method takes advantage of the fact that the outcome measure for individual patients is likely to be strongly correlated to the baseline value. For example, in the REPAIR trial an estimate of the glomerular filtration rate of the transplanted kidney from the living donor is available and is expected to be correlated to the kidney function at one year following transplantation. This information was used in the analysis of the data.
...
If patients are lost to follow-up for some endpoint then this reduces the effective sample size. For example, if two percent of patients in the UK PACE trial were expected to be lost to follow-up for mortality then the effective sample size of one thousand nine hundred twenty-six would be reduced by two percent (although in the UK, the Office of National Statistics can notify the trial when a patient dies).

Suppose the proportion expected to be lost to follow-up is 'Q' then, in order to adjust for this, the total sample size needs to be multiplied by one divided by one minus Q. For example, the UK PACE trial would need to be increased to one thousand nine hundred twenty-six times one divided by zero point nine eight equals one thousand nine hundred sixty-six.
...
Correction formula: multiply the required sample size by one minus Q one minus Q two squared

For example, suppose in the UK PACE trial:

One percent of patients randomised to the single chamber group received the dual chamber treatment Two percent of patients randomised to the dual chamber group received the single chamber treatment

The required size equals one minus zero point zero one minus zero point zero two squared one thousand nine hundred twenty-six equals two thousand forty-eight
...
One simple adaptation to the formula introduced on the previous page relates to placebo controlled trials where a proportion, Q, on the active therapy stop taking their medication. For the purposes of the sample size adjustment, the proportion who stop their placebo medication is less important as the randomised treatment contains no active ingredient expected to affect outcome.
...
As mentioned, certain assumptions are made but the adjustments do provide useful guidance.


## Eight point four Other adjustments

A trial which randomises patients to two treatment groups of equal size is usually the most efficient design in terms of total numbers required. However, it is sometimes the case that more patients are randomised to one group than the other.

Suppose patients are to be randomised to two groups in the ratio R to one. Then the total sample size required for equal numbers needs to be increased as follows:

Multiply the sample size by

R plus one squared divided by four R. For example, in the UK PACE trial, suppose the investigators wanted to randomise patients in the ratio two to one to the dual chamber treatment. The total sample size would need to be increased as follows:
...
· Trials of more than two treatments require more patients so some thought is necessary as to any benefits of including more than two treatment groups. One possible approach would be through the use of a factorial design.

In this session the main techniques have been covered to calculate sample size when comparing percentages and means for a clinical trial where each individual is randomised. Similar considerations are needed when estimating the required sample size for other measures of effects, e.g. rates which account for varying follow-up times, however a detailed discussion of these formulae is beyond the scope of this module but included in the articles in the references.

In addition, other sample size considerations are required depending on the trial design, e.g. when the aim is to show one treatment is equivalent or non-inferior to another treatment or when groups or clusters are randomised rather than individuals. These will be discussed in more detail later in the module.
